...
function prepare(level) {
  ...
  const prepared = { patterns, caps, starts, total, mix: findMix(total) };
  CACHE.set(level, prepared);
  return prepared;
}
function uniqueCapacity(level) {
...
function generateUniqueChatTurn(level, id, bankSize = 1e6) {
  ...
  const { patterns, starts, total, mix } = prepare(level);
  const local = Number(BigInt(index) * BigInt(mix) % BigInt(total));
  let lo = 0;
...
// scripts/check-unique.ts
var levels = process.argv[2] ? [Number(process.argv[2])] : [1, 2, 3, 4, 5, 6];
var sample = Number(process.argv[3] || 1e6);
